...
namespace Pong
{
	...
	std::shared_ptr<d2d::GameState> MainMenu::Update(float deltaSeconds)
	{
		if(m_quit)
			return nullptr;

		if(m_goPlay)
			return std::make_shared<Gameplay>(m_twoPlayerGame);	

		return shared_from_this();
	}
	void MainMenu::MakeButtonRect(const d2d::TextBlock& text,
									  d2d::Rect& rect)
	{
	...
	void MainMenu::DrawButton(const d2d::TextBlock& textBlock, bool highlight)
	{
		d2d::Rect buttonRect;
