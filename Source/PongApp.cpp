...
namespace Pong
{
	bool PongApp::Init()
	{
		...
		FILE *file;
		file = new FILE;
		if (freopen_s(&file, m_errorFilename.c_str(), "w", stderr))
			std::cerr << "Warning: Failed to redirect stderr to" << m_errorFilename << std::endl;
		...
		// Start Intro
		m_currentStatePtr = std::make_shared<IntroGameState>();
		return true;
	}
			...
			switch(event.type)
			{
				...
				default:
					if(m_currentStatePtr)
						m_currentStatePtr->HandleEvent(event);
			}
		}
	...
	void PongApp::AddController(int deviceIndex)
	{
	...
	void PongApp::RemoveController(int joystickID)
	{
